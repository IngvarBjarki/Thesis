...
def computer_1(X, y, theta, weight_decay, q):
    #print(theta)
    ...
    q.put(gradientDescent(X, y, theta, learning_rate, m, numIterations))
    #return(gradientDescent(X, y, theta, learning_rate, m, numIterations))
    

def computer_2(X, y, theta, weight_deca, q):
    # X is the data attributes and y are the targets
...
if __name__ == '__main__':
    ...
    for n in total_amount_of_data_intervals:  
        ...
        if is_multiProcessing:
            ...
            #computer_1_gradient.start()
            #computer_2_gradient.start()
            q1 = Queue()
            q2 = Queue()
            for i in range(num_rounds):
                c1 = Process(target=computer_1, args=(X_train1[:n], y_train1[:n], theta, weight_decay, q1))
                c2 = Process(target=computer_2, args=(X_train2[:n], y_train2[:n], theta, weight_decay, q2))
                #print('boom',1)
                c1.start()
                #print('bamm')
                c2.start()
                c1.join()
                c2.join()
                computer_1_gradient = q2.get()
                computer_2_gradient = q1.get()
                #print('c1', computer_1_gradient)
                #print('c2', computer_2_gradient)
                #theta = np.array([i for j in range(len(X[0]))])
                theta  = Gamma(0.5 * (computer_1_gradient + computer_2_gradient))
        else:
            for i in range(num_rounds):
                #laga nparray
                computer_1_gradient = computer_1(X_train1[:n], y_train1[:n], theta, weight_decay )
                computer_2_gradient = computer_2(X_train2[:n], y_train2[:n], theta, weight_decay )
                theta  = Gamma(0.5 * (computer_1_gradient + computer_2_gradient))
